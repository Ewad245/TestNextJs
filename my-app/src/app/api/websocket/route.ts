...
const RV32I_WS_URL = 'ws://localhost:8080'; // Update this with your actual RV32I CPU WebSocket URL
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to process WebSocket request', error: String(error) },
      { status: 500 }
    );
  }
}